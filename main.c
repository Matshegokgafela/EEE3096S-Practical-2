...
#define DELAY1 5000//3.2
#define DELAY2 256//1000000
...
int main(void){
  ...
  //decToBcd(24) //expect 36
  //decToBcd(128) //expect 296

  sprintf (buffer1, "%s %d %d \r\n", "24 and 81 in Binary Coded Decimal Respectively:", decToBcd(24), decToBcd(81));
  HAL_UART_Transmit(&huart2, buffer1, sizeof(buffer), 1000);

  //Convert back
  sprintf (buffer2, "%s %d %d \r\n", "10000001 and 00100100 in Decimal Respectively:", bcdToDec(129), bcdToDec(36));
  HAL_UART_Transmit(&huart2, buffer2, sizeof(buffer), 1000);

  HAL_Delay(10000);

  setTime(00, 00, 0, 6, 1, 1, 22);
  //setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
...
int epochFromTime(TIME time){
	...
	//months
	if (time.month == 1){ //31
		additions = ((time.dayofmonth-1)*86400);
	}
	if (time.month == 2){ //59
		additions = (30*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 3){ //90
		additions = (58*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 4){ //120
		additions = (89*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 5){ //151
		additions = (119*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 6){ //181
		additions = (150*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 7){ //212
		additions = (180*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 8){ //243
		additions = (211*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 9){ //273
		additions = (242*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 10){ //304
		additions = (272*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 11){ //334
		additions = (303*86400) + (time.dayofmonth*86400);
	}
	if (time.month == 12){ //365
		additions = (333*86400) + (time.dayofmonth*86400);
	}

	additions = additions + (3600*(time.hour)) + (60*time.minutes) + time.seconds;

	return EPOCH_2022 + additions;
}
